...
class write_FBD_files():
    def __init__(self,
        ...
        self.center_HSMRF = center_HSMRF
    ...
    def write_script(self, interval_ages, min_age):
        ...
        if self.translate is not None or min_age > 0.0:
            rho = 0.0
        if self.FBD_rate_prior == 'HSMRF' and self.center_HSMRF is False and self.padding[0] == np.inf and self.padding[1] == 0.0:
            ...
            scrfile.write('q()')
            scrfile.flush()
        elif self.FBD_rate_prior == 'HSMRF' and self.center_HSMRF is False and self.padding[0] != np.inf and self.padding[1] != 0.0:
            scr = "%s/%s/%s/%s/%s_FBDR_HSMRF.Rev" % (self.output_wd, self.name_file, 'FBD', 'scripts', self.name_file)
            scrfile = open(scr, "w")
            scrfile.write('######################################')
            scrfile.write('\n')
